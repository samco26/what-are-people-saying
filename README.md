...
- **Get Specific.** A small glass pill under the search card that opens in place into the full-width panel: the panel is laid out at full size and only its visible edge animates out from the pill, so it never stutters. On a wide screen it spreads into two columns: platform chips with logos and a time period with a custom from-and-to date range on the left, demographics on the right as rows of chips that combine, age, gender and region. Click to select, click again to clear. It can be set before a search. The refinements preview a selection and say so; they do not reanalyse anything.
