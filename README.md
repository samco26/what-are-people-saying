...
## Current checkpoint: BETA 0.13.0 — verified locally
...
The tests, TypeScript, production build and simulated desktop/mobile checks pass. This checkpoint has not been deployed or tested with paid live searches; no speedup, billed saving or live quality improvement is claimed. The last verified deployment is BETA 0.12.2, commit `3870bd8`.
...
Stars are a sentiment index: `1 + 4 × (positive + 0.5 × neutral) / accepted`, rounded to one decimal. This is not a submitted review average. No rating is shown for empty or entirely neutral evidence. There are no popularity-site thresholds, hidden priors or sample-size adjustments. The qualitative direction uses the same balance (positive at 60% or above, negative at 40% or below, otherwise mixed). These are product conventions, not statistically validated public approval measures. Confidence is low for fewer than 50 accepted opinions, one platform or fewer than five post groups, and at most medium otherwise because selection remains biased.
