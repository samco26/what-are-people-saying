...
## Current checkpoint: BETA 0.14.1 — warm gradient and frosted glass
...
The page background is the exact user-supplied black, plum and crimson JPEG gradient, with coral/crimson controls and dark translucent frosted glass. Search expands into the overall answer; recurring opinions float into place in the existing green, grey or red sentiment glass pills. Desktop shows up to six initially; on phones the whole list sits under the answer and scrolls within itself. The page stays fixed, with internal scrolling when necessary. Source buttons are logos only and replace the answer with a full-screen evidence view. A source view contains its logo, a full-width sentiment bar, and original post titles followed by sentiment-coloured comment excerpts. Each live post section links to its collected parent URL. Show more reveals the remaining analysed post groups without numeric totals. For general subjects the compact overall sentiment bar sits beside the platform icons; the four review categories use stars in the icon row. Tapping anywhere outside an expanded view closes it. Analysed counts and repeated platform headings are not displayed.
...
The direct path uses three AI requests:

1. A short cited web lookup starts **alongside** YouTube/X/Reddit collection with the cleaned original subject. The report is asked to stay under 120 words, with at most two web tool calls and a 1,200-token output ceiling. Collection keeps the existing platform/time-window limits and does not re-fetch after discovering a corrected name.
2. One request reads every collected item and resolves the identity/category from the cited report. Claims refer only to actual web citation annotations, and unsupported identity cannot reach the summary. Each opinion still has one required field, using `p/u/n/i` for positive/neutral/negative/irrelevant. The model proposes supported recurring themes; code validates references, removes duplicates, computes sentiment and selects source excerpts from checked themes. There is no separate AI source-selection output. The 16,000-token ceiling stays to avoid truncation; shorter requested output, not a lower ceiling, is the optimisation.
3. A short writer receives only validated themes/counts, dates and factual context (700-token ceiling). Raw comments are not sent twice. The summary and ratings use the same counts. Both analysis requests share the remaining time before the route's 55-second deadline. Fewer than eight accepted opinions skips writing and returns insufficient evidence; `MIN_ITEMS` may raise that threshold.

A conservative code rule routes one-word names, more than five words, questions, comparisons and conditional wording through the existing researched-query path. That path performs an additional extraction/planning request before collection, using supported aliases within the same source caps. It remains four AI requests. The rule does not detect every typo or ambiguous name: a direct search can miss discussion under a corrected spelling or alias, and analysis can reject wrong matches but cannot retrieve missing posts. Lookup failure preserves the original subject and general category with low confidence. No factual cache, streaming summary or new paid retrieval loop is added.
...
Parallel platform collection, compact numeric references and request-local response reuse remain in place. Short multiword searches use three AI requests and overlap research with collection; the cautious researched-query path uses four. Classification output uses compact labels and omits estimated percentages, unused positive/negative theme objects and representative-reference selections. Actual latency and total token use can still increase or decrease; request count alone does not prove a saving. The historical 0.10.3 production analysis timing of 11.95 seconds belongs to the previous pipeline.

Analysis failures return a content-free stage code and elapsed time; server logs omit inputs, social content, credentials and raw provider messages. Successful responses expose `Server-Timing` for `lookup`, `collection`, `analysis`, its separate `classification`/`summary` steps, `total`, and the `parallel`/`planned` flow. Lookup and collection overlap on the direct path, so their durations must not be added. The route requests a 60-second hosting limit. Relevance filtering happens after bounded collection; rejected items do not automatically trigger more paid searches. This can yield insufficient evidence even when more relevant discussion exists elsewhere. No social content is cached across searches.
...
- Current checkpoint: the prior BETA 0.14.0 suite of 70 tests, TypeScript and the production build passed using the bundled Node.js 24 runtime (the same commands behind the package scripts; npm is not on PATH). Browser checks used bundled Playwright because agent-browser is not installed. All five categories passed at 1440×900, 390×844 and 320×660 with equal-height controls, no page/row overflow, duplicate-quote removal, Back, clickable suggestions, reduced motion and no runtime errors. Provider responses and browser fixtures were simulated. The warm-gradient visual refresh uses the supplied JPEG with dark, translucent frosted glass and keeps the pre-existing green, grey and red sentiment indicators. Its local verification is recorded in PHASES.md. Deployed commit `91242eb` successfully to Vercel Production. The homepage served BETA 0.14.0 with the Tiffany AVIF at that earlier checkpoint. A live “iphone fold” search returned HTTP 200, product category, 81 accepted YouTube opinions, 11 X opinions and 7 themes in 24.79 seconds. Server timing confirms the parallel flow: research 8.609 seconds, collection 1.039 seconds (overlapping), classification 13.270 seconds, summary 1.812 seconds, total server time 23.736 seconds. This did not demonstrate a speedup over the earlier 24.67-second request; the evidence sample and model runtime differed. One fewer AI request does not establish lower billed cost, and no social content was persisted.
