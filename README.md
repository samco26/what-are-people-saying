...
`POST /api/consensus` accepts `{ "subject": "..." }`. Before anything is collected, one small OpenAI call (`src/lib/searchPlan.ts`, same model, about 400 input and under 400 output tokens) turns the subject into a **search plan**: the short name people use, a one-sentence interpretation, one to four exact phrases discussion would contain, distinctive keywords, words that mark a different meaning of the same name, and a plain YouTube search. The platform queries are built in code from those parts, so the model never writes operator syntax: X gets `("phrase" OR "phrase" OR keyword) -otherMeaning` plus the fixed `-is:retweet lang:en`, YouTube gets the plain words plus `-otherMeaning`, Reddit gets the short name. A short subject as typed always stays among the X terms. If the plan call fails, times out (6 seconds) or returns nothing, the subject is searched as typed, exactly as before. The interpretation is passed to the analysis as "Taken to mean" so ambiguous names are classified against one meaning. The plan lives for one request and is not stored.
