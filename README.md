...
- **Get Specific.** A small glass pill under the search card that grows in place into the full-width panel, slowly, and on a wide screen spreads outward into two columns: platform chips with logos and a time period with a custom from-and-to date range on the left, demographics on the right as rows of chips that combine, age, gender and region. Click to select, click again to clear. It can be set before a search. The refinements preview a selection and say so; they do not reanalyse anything.
