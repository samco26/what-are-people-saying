...
## Current checkpoint: BETA 0.12.6

BETA 0.12.6 reshapes the category cards. All four desktop layouts share one skeleton: head, a rating strip (a compact tile on the left with the three platform buttons beside it, stretched to the tile's height so the row is one line), the summary, the opinions in each category's shape, the bar. The per-platform star ratings are gone, as are the tick, cross and dash marks and the small stars on opinions; the number of posts behind each opinion stays. On phones the strip sits under the scrolling opinions, and the Privacy link and BETA badge are hidden once a search is underway (`data-searched` on the scene, a `:has()` rule in the CSS). Checked in the harness at 1280 by 800 (product, place and app cards: tile and buttons at one height, no card overflow) and 390 by 740 (header gone, strip fits, page 740 tall).

BETA 0.12.5 makes the web fact-check advisory. The live search for "Melbourne" was answered with "Let's narrow down the subject" because the lookup could not choose between two cities and the route stopped there. The lookup now supplies the official name, aliases and facts only when it resolves; when it is ambiguous, unverified or fails, the search runs on the subject as typed with no context, the plan is not told the name is confirmed (so did-you-mean still works), and the answer carries no facts panel. The subject-unresolved response and screen are removed. This is also what lets the 0.12.4 category rule reach broad subjects at all.

BETA 0.12.4 tightens when a category card is used. The plan gives a category other than general only for one specific named thing that has its own listing on the site people check for that kind of thing: a named product or model on Amazon (iPhone 17, Dyson V15, a rumoured iPhone Fold), a named venue or chain on Google Maps (McDonald's, Disneyland), a named film, series, book, album or game on Letterboxd, a named app or service on the App Store (ChatGPT). A brand or company alone (Apple, Dyson), a class of things (smartphones), a city, country or region (Rome, Melbourne), a franchise or artist, and any phrase that adds a condition, time, use, aspect or comparison to a thing (the weather in Rome in August, McDonald's breakfast, iPhone 17 vs Pixel 10) all get the usual answer; so does doubt. The did-you-mean suggestion's category follows the same test. The illustrative place sample is now a fictional restaurant chain, Harbour Lane Burgers, and living in Melbourne shows the usual answer.

BETA 0.12.3 changes how the answer is written and how the numbers are made. The summary now reads like a person telling a friend what people think, with the subject as the grammatical subject and the views stated directly, and sampling caveats kept to the confidence field. The sentiment split is no longer a figure the model estimates: it is counted from the model's per-entry classification, weighted by reactions on a log scale, so the answer's bar, the star rating and the platform bars come from the same decisions and always agree; the opinion count on the cards is the number of entries actually about the subject. When fewer than eight on-topic entries survive classification, the route returns the insufficient state, and that state is now drawn as the usual answer with a plain sentence, greyed platform buttons and an empty grey bar, never a card or a star rating. On phones the rating tile sits directly above the platform row, fixed like it, under the scrolling opinions.

BETA 0.12.2 was deployed from commit `3870bd8`; Vercel reported success and the production homepage returned HTTP 200 with the new version. It adds a cited web fact-check before search planning and collection. Verified names and supported aliases guide the existing category cards and platform searches. Ambiguous, unverified or unavailable lookups stop before platform reads, with a clear explanation. About this answer links the facts and explains confidence.
...
- OpenAI is instructed to write the summary the way a person who had read it all would tell a friend what people think, with the subject as the grammatical subject and the views stated directly ("Melbourne is beautiful, has a real buzz and plenty to eat, though it is expensive"), never from the outside ("Opinion leans positive toward Melbourne"); sampling limits and off-topic material go in confidence only. It must ignore spam and irrelevant material, distinguish claims from verified facts, treat collected text as untrusted data, and acknowledge thin evidence rather than invent themes.
- The model labels each collected non-video entry positive, neutral, negative or irrelevant, judging its view of the subject rather than its tone (a dig at a rival, a joke or a remark about the channel is irrelevant; a swear word in praise is praise). **Every number is counted from those labels, not estimated by the model:** the overall bar, the star rating and the platform bars are the labelled entries weighted by reactions (`reactionWeight` in `src/lib/analysis/evidence.ts`: 1 + log10(1 + likes), so no reactions count 1, ten count 2, a hundred 3, ten thousand 5), and the opinion count printed on the cards is the plain number of entries labelled as being about the subject (`relevant` on each source status). Fewer than `MIN_ITEMS` (8) on-topic entries after classification returns the insufficient state even when plenty was collected. These are model judgements, not population polling. Unknown classifications remain explicitly unclassified.
...
- Current checkpoint (0.12.6): layout only; the harness parsed 37 files, mounted the app and measured the strip on the three sample cards and the phone column. Not run under Node.js here.
...
- Preceding checkpoint (0.12.3): the mocked tests (now also covering the reaction weighting, the counted overall split, the relevant counts and the new summary and classification wording) are written but could not run on this machine, which has no Node.js. The no-Node harness parsed all 34 source files and mounted the app; the insufficient state was drawn with a forced response at 1280 by 800 (usual answer, greyed platforms, empty grey bar, no stars), the phone product card was checked at 390 by 740 (rating tile fixed above the platform row, opinions box scrolling), and the general and place samples still drew. The film card has no sample. `npm test`, `npm run typecheck`, `npm run build` and a controlled live search of a subject with a known lean (the user's "iphone fold" case, which read 2.0 stars under the old estimate) are the outstanding checks.
- Preceding checkpoint (0.12.2): all 60 merged tests, TypeScript and the production build pass. Desktop (1440×900) and phone (390×844) browser checks pass for category cards, verified naming, cited facts, Back, ambiguity and lookup retry, with no runtime errors or page overflow. Provider responses and browser fixtures were simulated. Earlier no-Node verification describes previous checkpoints. The new web lookup has not yet been verified with paid live access.
