...
The user-approved X budget is now 20 posts per search, at most US$0.10 in post-read charges at the published US$0.005 rate, excluding AI. The code clamps old higher environment settings to 20; X still makes one request, with no pagination or extra reply searches. Twenty-two tests and the production build pass. The first local build encountered empty duplicate type-package directories in `node_modules`; removing those empty directories restored the build without dependency or application changes. Earlier live 50-post checks below describe the previous limit.
