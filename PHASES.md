...
X now uses official full-archive search with its own 3/12/36-month empty-result fallback, even when other sources have sufficient evidence. It stops at the first nonempty response and keeps the 20-post total cap. Twenty-seven tests and the production build pass, including the 12-month and 3-year paths, empty results, access/billing failures, timeout cancellation, per-request reuse, and the wider answer window. Production archive access remains to be checked; earlier successful X searches below used the recent endpoint.
