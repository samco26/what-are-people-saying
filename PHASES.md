...
BETA 0.10.1 improves subject accuracy within milestones 6 and 10. A bounded, server-side web lookup resolves the official name and supported aliases before platform collection, supplies cited dated facts separately from opinion evidence, and explicitly stops on ambiguity, missing verification or lookup failure. Platform aliases share existing read limits. About this answer exposes linked facts and the confidence explanation. Forty-four simulated-provider tests, TypeScript and the production build pass, including the complete route from alias resolution through collection and analysis. Desktop (1440×900) and mobile (390×844) production-build browser checks passed for the official-name label, linked facts, confidence explanation, Back, ambiguity and retry, with no runtime errors or document overflow; these used fictional responses. No dependencies were added and no social content was stored. This checkpoint is local only: no new live lookup, deployment, latency or billed-cost verification is claimed. Temporal distinctions and factual interpretation still depend on the model and require live quality checks.
