...
Rules the user set along the way: no scrolling of the page or the card on desktop; the card widens outward to 880px rather than growing down; the opinion labels are all "Common opinions"; no "Read as … change" chip and no "Worth buying"-style verdict badges anywhere; stars are a deeper cut of the peach (`--star`), since a rating is neither a sentiment nor a chosen control; the approval share is always printed beside the stars so nobody reads them as an IMDb score; General view, a small glass pill top right of the card body on desktop only, returns to the usual answer. On phones every category shares one column (rating tile, summary, the common opinions in a box that scrolls in place with the same fade the pill list uses, each platform's rating in a row, the bar); no title (it is in the field), no General view. Did you mean: an ambiguous name gets the usual answer and bold italic ghost text after the typed word, accepted with Tab or a click. The "When people go" strip on the place mockup was not implemented: nothing collected supports it.
